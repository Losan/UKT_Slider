...
?>
		<br />
		<div style="height:357px;width:922px;overflow: hidden; z-index:5; margin-left:-170px;">
		...
		<?php	
			...
			while ( $ukt_loop->have_posts() )
			{
				...
				$more_info = get_post_custom_values('more_info_link');
				$order_now = get_post_custom_values('order_now_link');
			...
			<li class="li_slideshow active" id="image_<?php echo $i; ?>" style="float:left;height:357px;width:922px;margin-left:<?php if($i==1) echo "170"; else echo "-2";?>px;">
				<ul style="list-style-type: none;margin: 0;padding: 0;width:800px;">
					...
					<li style="float:left;margin-right: -12px">
							<div onclick = "window.open('<?php echo $right_link[0] ?>')" style="cursor:pointer; width:467px;height:357px;background-image:url('<?php echo $right_path[0]; ?>');">
								...
								<div onclick = "window.open('<?php echo $more_info_link[0] ?>')" style="position:absolute;background-image:url('<?php echo $more_info; ?>');width:180px;height:42px;margin-top: 269px;margin-left: 55px;">
									<a href="#" class="button-link" style="color:white;line-height: 18px;padding: 12px 30px 12px 48px;display: block;width: 120px;font-weight:bold;">MORE INFO</a>
								</div>
								<div onclick = "window.open('<?php echo $order_now_link[0] ?>')" style="position:absolute;background-image:url('<?php echo $order_now;?>');width:180px;height:42px;margin-top: 269px;margin-left: 244px;">
									<a href="#" class="button-link" style="color:white;line-height: 18px;padding: 12px 30px 12px 43px;display: block;width: 120px;font-weight:bold;">ORDER NOW</a>
								</div>
							</div>
					</li>
				</ul>
